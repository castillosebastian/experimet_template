...
```{r}
# roc curve
covid_roc_curve <- lgbm_test_preds_tun %>% roc_curve(resultado, `.pred_1`)
...
sub = readRDS(paste0(RAW_DATA_DIR, "/covid_utn2022_unknown.rds"))

lgbm_final_tun <- lgbm_final_wflow_tun %>% fit(covidtrain) 

sub$resultado <- predict(lgbm_final_tun, covidtest)$`.pred_class`
